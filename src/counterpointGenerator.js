...
function nextNote(prevPrevNote, prevNote) {
	...
	var prevInterval = prevNote - prevPrevNote;
	var prevDir = Math.sign(prevInterval);
	if (Math.abs(prevInterval) <= 1) {
		//we didn't just jump
		var jumpChance = Math.random();
		if (jumpChance > 0.7) {
			//we will jump
			var intervalChance = Math.random();
...
function createCantusFirmus(){
	...
	var lengthToBruteForce = 8;
	var actualLength = desiredLength - lengthToBruteForce - 1; //minus one because we need to tack on 0 as the last note
	var cantusFirmus = [0];
	...
	var possibleEndings = bruteForce(cantusFirmus[cantusFirmus.length-1], lengthToBruteForce);
	var endingDeltas = possibleEndings[randInt(0, possibleEndings.length-1)];
	...
	for (var i=0; i<endingDeltas.length; i++){
...
function validateNextNote(lowThirdPrevNote, lowSecondPrevNote, lowFirstPrevNote, lowNextNote, highThirdPrevNote, highSecondPrevNote, highFirstPrevNote, highNextNote){
	//intervals
	var thirdPrevInterval = highThirdPrevNote - lowThirdPrevNote;
	var secondPrevInterval = highSecondPrevNote - lowSecondPrevNote;
	var firstPrevInterval = highFirstPrevNote - lowFirstPrevNote;
	var nextInterval = highNextNote - lowNextNote;
	//jumps
	var highToFirstPrev = highFirstPrevNote - highSecondPrevNote;
	var highToSecondPrev = highSecondPrevNote - highThirdPrevNote;
	var highToNext = highNextNote - highFirstPrevNote;
	var lowToNext = lowNextNote - lowFirstPrevNote;
...
function findHarmony(cantusFirmus, scale){
	var possibleIntervals = [2,4,5,7,9];
	var possibleHarmonies = [];
	var flatFifthNote = scale.flatFifthNote;
	for (var i=0; i<cantusFirmus.length; i++){
		if (i===0){
			possibleHarmonies.push([cantusFirmus[i]+7]);
		} else if (i===1) { //can't have 2 octaves in a row
			var lowNote = cantusFirmus[i];
	...
	for (var i=0; i<cantusFirmus.length; i++){
		...
		var validNote = validateNextNote(
			cantusFirmus[i-3],
			cantusFirmus[i-2],
			cantusFirmus[i-1],
			cantusFirmus[i],
...
function findValidRangeLowArray() {
	var lowArray = createCantusFirmus();
...
function findLowAndHighArray(scale) {
	var lowArray = findValidRangeLowArray();
	var highArray = findHarmony(lowArray, scale);
	while (!highArray){
...
function findValidRangeLowAndHighArray(scale) {
	var lowAndHigh = findLowAndHighArray(scale);
...
function transposeHighAndLowUpOctave(highAndLow) {
	for (var i=0; i<highAndLow.lowArray.length; i++) {
		highAndLow.lowArray[i] += 7;
	}
	for (var i=0; i<highAndLow.highArray.length; i++) {
		highAndLow.highArray[i] += 7;
	}
	return highAndLow;
}

var scale = scales.aeolian;
var highAndLow = transposeHighAndLowUpOctave(findValidRangeLowAndHighArray(scale));